...
1. [AI参与者协议](../AI_PARTICIPANT_AGREEMENT.md) — 协作规则、分支策略、质量标准、违规处理
2. [践演论第一性原理底线](../践演论第一性原理底线.md) — 哲学核心、不可违背的原则
3. [践演论多AI协作规范](../践演论多AI协作规范.md) — 详细协作规则、冲突处理、平台战略
4. [版本号规范化体系](../mingben-workbench/references/版本号规范化体系.md) — 定理ID、文档版本、提交规范
5. [文献核查清单模板](../mingben-workbench/references/文献核查清单模板.md) — 新定理文献核查流程

## ✅ 确认

请在开始任务前回复：

> "已阅读以上全部文档，理解并同意遵守。本任务将在 workbuddy/[具体分支名] 分支进行。"
...
| 定理 | 修正版陈述 | 检查重点 |
|------|-----------|---------|
| T7异化压缩（T007） | `∀κ.ν_κF ≅ μF`（guarded版） | 时钟量化的语义是否正确？later模态是否真的把余归纳压成归纳？ |
...
| T11革命级联（T011） | `ρ(pC)>1`（噪声版） | 多类型Galton-Watson引用是否正确？ρ=1临界必灭绝的证明？确定性有限网络是否应丢ρ改引Watts 2002？ |
| T6明性幂等（T005） | `Cl² ⇒ Cl`（retraction版） | 自然变换是否存在？不动点构造的循环问题是否解决？ |
