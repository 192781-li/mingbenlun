...
Proof.
  ...
  induction Q; intros y T Gamma H Hy; simpl.
  - (* PVar n *)
    destruct (n =? 0) eqn:E.
    + (* n = 0: subst_var y 0 (PVar 0) = PVar y *)
      rewrite (Nat.eqb_eq n 0) in E. subst n.
      inversion H as [Gamma0 x T0 Hget | | | | | | |].
      * (* ty_var *)
        simpl in Hget. injection Hget as Hget. subst T0.
        apply ty_var. exact Hy.
      * (* 其他case不可能，因为PVar 0只能由ty_var推导 *)
        all: exfalso; simpl in Hget; discriminate Hget.
    + (* n > 0: subst_var y 0 (PVar (S n')) = PVar n' *)
      destruct n as [| n'].
      * discriminate E.
      * inversion H as [Gamma0 x T0 Hget | | | | | | |].
        ** (* ty_var *)
          simpl in Hget. apply ty_var. exact Hget.
        ** (* 其他case不可能 *)
          all: exfalso; simpl in Hget; discriminate Hget.
  - (* PZero *)
    inversion H; subst. apply ty_zero.
  - (* PTau Q *)
    inversion H; subst. apply ty_tau. eapply IHQ. eassumption. exact Hy.
  - (* POut x y0 Q *)
    inversion H; subst.
    eapply ty_out.
    + eassumption.
    + eassumption.
    + eassumption.
    + eapply IHQ. eassumption. exact Hy.
  - (* PIn x Q *)
    inversion H; subst.
    eapply ty_in.
    + eassumption.
    + eassumption.
    + eapply IHQ. eassumption. simpl. exact Hy.
  - (* PPar Q1 Q2 *)
    inversion H; subst.
    eapply ty_par.
    + eassumption.
    + eapply IHQ1. eassumption. exact Hy.
    + eapply IHQ2. eassumption. exact Hy.
  - (* PRes Q *)
    inversion H; subst.
    apply ty_res. eapply IHQ. eassumption. simpl. exact Hy.
  - (* PRep Q *)
    inversion H; subst.
    apply ty_rep. eapply IHQ. eassumption. exact Hy.
Qed.
