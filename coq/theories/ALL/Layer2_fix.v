(* =====================================================================
   ALL_Layer2_fix.v
   Layer 2（修正版）：替换引理 + subject reduction + Progress
   基于用户提供的 ALL_Layer2.v（DeepSeek 盲写，未编译）修正。
   修正内容：progress 的 ty_par 分支(B)、subject_reduction 的 bullet 结构(C)
            及 s_par_l/s_par_r 缺 step 前提(D)、ty_res 缺 step 前提(E)。
   状态：修正版，未经 coqc 编译，需在用户真机验证。ren_typed 仍依赖 Layer1 的 Admitted。
   Author: workbuddy
   ===================================================================== *)
...
(* =====================================================================
   0. 前置事实
   (1) L1 没有 step/value —— L2 补定义。
   (2) 在 L1 线性 split 下通信 redex 不可定型（split_contra），
       subject_reduction 的 s_comm 分支由矛盾闭合；
       msubst_typed 不被任何归约使用，为 L3 准备。
   ===================================================================== *)
...
(* =====================================================================
   ...
   修正(C)：inversion Hs 已闭合 ty_zero/ty_out/ty_in，故仅剩 4 个顶层目标
            （ty_tau / ty_par / ty_res / ty_rep），bullet 数对齐为 4。
   修正(D/E)：s_par_l / s_par_r / ty_res 须补上 step 前提（Hs1 / Hs2）。
   ===================================================================== *)
...
Proof.
  ...
  - (* ty_tau：s_tau 使 P' = 内层 P，定型不变 *)
    exact H.
  - (* ty_par *)
    + (* s_comm：通信 redex 在线性 split 下不可定型，由矛盾闭合 *)
  ...
  - (* ty_res：s_res 使 P → P' *)
    eapply ty_res. eapply IH. eassumption. exact Hs1.
...
(* =====================================================================
   6. Progress（进展性）
   修正(B)：ty_par 的 Hv1/P2' 分支应为 s_par_r P Q P2' Hs2，
            原稿误用未绑定的 P1' 与错误构造 s_par_l。
   ===================================================================== *)
