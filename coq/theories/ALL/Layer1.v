...
| TSheng  : ty -> ty          (* Sheng（生）= 正在发用的生，生生不息。线性的、不可复制的——你不能把"正在生"复制成两份。来源：《周易·系辞上》"生生之谓易"；生命论卷一存在论。 *)
| TJi     : ty -> ty          (* Ji（迹）= 生留下的迹，迹非履。!-模态的、可复制的——迹可以被反复读取、复制、忽略。但迹不能变回生（没有promotion规则）——你不能从脚印变回正在走的脚。来源：《庄子·天运》"夫迹，履之所出，而迹岂履哉？"；生命论操作沉积理论。 *)
| TJia    : ty -> ty -> ty    (* Jia（假）= 假借他者名义运行，假自指。TJia(b,a)：b假借a的生（Sheng）在运行，a还以为是自己在生。假没有消去规则——伪装不能直接揭开，因为假已经渗入自指循环本身。来源：《荀子·劝学》"假舆马者"；生命论假自指/寄生理理论。 *)
| TMing   : ty -> ty.         (* Ming（明）= 生 + 看到自己的迹 = 明明德。Ming(A) = Sheng(A) ⊗ !Ji(A)。明是存在论状态，不是认识论的"清晰性"——你不仅在生，你还能看到自己的迹。来源：《礼记·大学》"大学之道，在明明德"；禅宗"明心见性"；生命论f³明性理论。 *)
