...
Proof.
  (* 为真路线（SOP指定）：见证 G23 = setby f (repeat None L) 0，
     f 逐位 G2 优先、G3 兜底（工具引理 H23_val 已 Qed，line 2229-2265）。
     空位形态可选可收摄（结晶012），重组不被空位阻塞。
     旧证明（S01 commit 300a95b）用 assoc_build 构造 G23，
     但 assoc_build 在 G=[] 时返回 []，导致 split [] G2 G3
     在 get G2=Some None 且 get G3=Some None 时不成立（左支需G2=None，右支需G3=None）。
     重置为 Admitted，交 DS 用 H23_val 正确构造重新主谋闭环证明。 *)
Admitted.
(* ---------------------------------------------------------------------
   10. Progress
   --------------------------------------------------------------------- *)
