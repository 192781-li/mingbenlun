...
(i) \textbf{对象级}：取$A=\nu F_2$（定理\ref{thm:nu-lift-data}）或$A=\mu F_2$，得$\nu F_2\to\bang\nu F_2$、$\mu F_2\to\bang\mu F_2$不存在——阴化方向被阻；\\
...
本定理与种子定理\ref{thm:no-A-to-bangA}（不存在$A\to\bang A$）同族：$G^\omega$中的$\bang$穿透若存在，将诱导底层$\mathcal{C}$中的$\bang$-结构（具体诱导构造待补充，与定理\ref{thm:bang-no-ptc}同条件），与种子定理的精神一致。
\end{remark}
...
\begin{proof}
依定义\ref{def:gomega}与定理\ref{thm:gomega-compact}的递归类型构造，$f^1\to f^2$是质变（从无模型到有模型）；$f^2\to f^3$是质变（从被模型支配到看见模型是模型）；$f^3\to f^4$不是质变——$f^3$已能对象化任何模型（包括自身的模型），再嵌套一层不增加新的区分能力。在$G^\omega(\mathcal{C})$中，$X_3$的递归类型允许任意深度的$\ulcorner\ulcorner\cdots\urcorner\urcorner$嵌套，$X_4$不增加表达力。\qed
\end{proof}
