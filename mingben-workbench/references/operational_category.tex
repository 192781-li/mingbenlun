...
\begin{remark}[种子定理派生]
定理\ref{thm:nu-no-lift-life}与\ref{thm:mu-no-lift}是种子定理\ref{thm:no-A-to-bangA}经不动点结构的推广：种子定理说$A\to\bang A$不存在，此处说$\nu F_2\to\bang\nu F_2$和$\mu F_2\to\bang\mu F_2$不存在。
\end{remark}
...
\begin{theorem}[!在$G^\omega(\mathcal{C})$中仍不穿透]\label{thm:bang-gomega}\lvC
在$G^\omega(\mathcal{C})$中不存在$\bang\nu F_2\to\nu\bang F_2$。$G^\omega(\mathcal{C})$中的态射是$\mathcal{C}$中的生产性Mealy机，若该提升存在则底层$\mathcal{C}$中也存在，与定理\ref{thm:bang-no-ptc}矛盾（当$\mathcal{C}$满足状态唯一可追踪性条件时）。
\end{theorem}

\begin{remark}[种子定理派生]
本定理是种子定理\ref{thm:no-A-to-bangA}在$G^\omega$紧闭合范畴中的推广：若$\bang$穿透$\nu F_2$，则底层$\mathcal{C}$中$A\to\bang A$存在，矛盾。
\end{remark}
...
\textbf{此处"共享同一结构"的精确含义}：每条命题都可从种子定理\ref{thm:no-A-to-bangA}（不存在$A\to\bang A$）或其经不动点/GoI结构的推广（定理\ref{thm:dichotomy}、\ref{thm:no-bang-stream}、\ref{thm:bang-gomega}）与线性逻辑公理的一个小推导中得到；第1、6、8条虽为[诠释]，其数学载体同样来自定理\ref{thm:no-A-to-bangA}/\ref{thm:recursive-characterization}/\ref{thm:no-bang-stream}/\ref{thm:bang-gomega}。
