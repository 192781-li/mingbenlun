...
\begin{theorem}[自我模型是!-模态的]\label{thm:self-model-bang}
...
\begin{corollary}[意识形态的存在论基础]
因为自我模型是$\bang$-模态的，它可以从外部被植入、复制、传输。意识形态、经文化、身份政治的运作基础正在于此：系统给人一个$\bang$-自我模型，这个模型可被大规模复制。
\end{corollary}

\begin{theorem}[异化=错认名字为过程]\label{thm:alienation-mistake}
异化发生在$f^2$层：当$\bang$-模态的自我模型被错认为线性运行过程$X$本身时，系统按沉积模型运行自己（$S=f_{\bang}(S)$），而非自己运行自己（$S=f(S)$）——阴主阳从。
\end{theorem}

\begin{proof}
运行中的$X$是线性的（每步消耗、不可复制，定理\ref{thm:no-bang-stream}）。$\bang\ulcorner f\urcorner$是沉积的、固定的、可外部植入的。将后者等同于前者，运行权即交给模型：此后操作不产生新的$X'$，只重复$\bang$-模型的规定。这就是经文化——模型停止更新，成为$\bang$的纯收缩（空转）。
\end{proof}

\begin{theorem}[明性=看见名字是名字]\label{thm:mingxing}
明性（$f^3$）是维持$X$（线性运行）和$\bang\ulcorner f\urcorner$（沉积模型）之间区分的能力。不是没有模型（$f^2$必要），是不把模型等同于自己。
\end{theorem}

\begin{proof}
$f^3$的第三条反馈回路将自我模型对象化：系统看到模型是$\bang$-模态的（固定的、可复制的、可能外部植入的），运行是线性的（每步更新的、不可复制的、自己的）。这个"看见"使系统能质疑被植入的模型、更新过时的模型、在模型和活操作间保持张力。
\end{proof}

\begin{theorem}[$f^2$的哥德尔困境]\label{thm:godel-f2}
能完全表示自身的$f^2$系统包含不可判定命题。$f^3$践演是出路：哥德尔句子在$f^2$内不可证，但在$f^3$层通过践演判断（公理\ref{ax:performative}，元层面操作）被体知——不是在模型内证明，是在运行中看见模型的限度。
\end{theorem}

\begin{remark}[$\alpha$的GoI定义]
在GoI执行公式中，$\alpha$是$U_{11}$矩阵的反馈嵌套深度：$f^1$有一层迭代$\sum(U_{11})^n$，$f^2$的$U_{11}$本身含反馈（迭代中的迭代），$f^3$再深一层。$M=\alpha\cdot T\cdot N$中$\alpha$加权意义的深度：$f^1$的意义是维持，$f^2$是自我理解，$f^3$是自觉地活出完整。
\end{remark}

\section{可靠性与量子对应}

\begin{theorem}[可靠性]\label{thm:soundness}
...
\begin{corollary}
操作$\nu F_2$型资源的证明不能被promotion（不能变成可复制的教条）。这是定理\ref{thm:no-bang-stream}的证明论对应——关于生命过程的证明本身必须是"活的"，不能冻结为可复制的定理。
\end{corollary}

\begin{theorem}[不可克隆是推论]\label{thm:no-cloning-corollary}
在$\mathbf{FdHilb}$（有限维Hilbert空间与线性映射）中，量子对象（维数$\geq2$）无自然收缩$\Delta:A\to A\tensor A$，量子过程构成生产性迹范畴。由定理\ref{thm:bang-no-ptc}，$\bang$不提升到量子交互流处理器——这就是Wootters-Zurek不可克隆定理。
\end{theorem}
...
\begin{theorem}[定理21严格化]\label{thm:mu-rigorous}
...
\begin{definition}[$G^\omega(\mathcal{C})$]
...
\begin{theorem}[$G^\omega(\mathcal{C})$是dagger紧闭合范畴]\label{thm:gomega-compact}
$G^\omega(\mathcal{C})$是范畴（定理34），且是dagger compact closed范畴（定理35）：对偶$(A^+,A^-)^*=(A^-,A^+)$，unit/counit由$\mathcal{C}$中的弯线给出，蛇方程由Yanking公理保证。$G^\omega(\mathcal{C})$还有递归类型（定理36）：流对象$A^\omega=\nu X.(A\tensor X)$是合法对象，$\nu F_2$（生命流类型）在$G^\omega(\mathcal{C})$中可解释。
\end{theorem}
...
\begin{theorem}[!在$G^\omega(\mathcal{C})$中仍不穿透]\label{thm:bang-gomega}
在$G^\omega(\mathcal{C})$中不存在$\bang\nu F_2\to\nu\bang F_2$（定理37）。$G^\omega(\mathcal{C})$中的态射是$\mathcal{C}$中的生产性Mealy机，若该提升存在则底层$\mathcal{C}$中也存在，与定理\ref{thm:no-bang-stream}矛盾。
\end{theorem}
...
\begin{theorem}[$f^3$是不动点]\label{thm:f3-fixedpoint}
层级提升操作$L$（增加一条反馈嵌套）在第三层达到不动点：$L^3=L^n$对所有$n\geq3$。$f^3$的状态类型$X_3$满足$X_3\cong\bang(X_2\tensor X_3)$，已可包含任意深度的模型嵌套；第四条反馈线在类型上同构于第三条的递归展开。
\end{theorem}

\begin{proof}
$f^1\to f^2$是质变（从无模型到有模型）；$f^2\to f^3$是质变（从被模型支配到看见模型是模型）；$f^3\to f^4$不是质变——$f^3$已能对象化任何模型（包括自身的模型），再嵌套一层不增加新的区分能力。在$G^\omega(\mathcal{C})$中，$X_3$的递归类型允许任意深度的$\ulcorner\ulcorner\cdots\urcorner\urcorner$嵌套，$X_4$不增加表达力。
\end{proof}
...
\begin{theorem}[慢性死亡]\label{thm:slow-death}
当$N<1$时，即使过程永远运行（生产性），总意义有限：$M=\alpha/(1-N)<\infty$。系统在"慢性死亡"——每步意义衰减，趋近于零。
\end{theorem}

\begin{theorem}[萃取加速异化]\label{thm:extraction-alienation}
$W$越高则$N$越低。萃取把产出从反馈线抽走，$U_{22}$输入减少，谱半径下降。$W=1$时$U_{22}=0$，$N=0$，过程立即死亡。
\end{theorem}

\begin{theorem}[明性抵抗磨损]\label{thm:mingxing-resists}
$C>0$（认清多于接受）时$N$趋向$\geq1$；$C<0$时$N$趋向$<1$。f³运行时线性过程$X$保持自我更新；f²把!-模型当自己时$U_{22}$被!-收缩削弱。
\end{theorem}

\section{完全性}

\begin{theorem}[有限片段完全性]\label{thm:completeness-finite}
在有限非交互片段（$F_1$型，$\mu$不动点）中，GoI解释对MELL片段完全：每个nilpotent连线对应一个证明（Girard 1989，Danos-Regnier 1995）。
\end{theorem}

\begin{theorem}[生命过程超越证明]\label{thm:life-beyond-proof}
在余归纳片段（$\nu F_2$型）中，存在生产性Mealy机不对应任何有限证明。生产性流处理器的集合是连续统，有限证明的集合是可数的。
\end{theorem}
...
\begin{theorem}[Hilb$^\omega$是PTC]\label{thm:hilb-ptc}
...
\begin{theorem}[Hilb$^\omega$完整PTC]\label{thm:hilb-ptc-full}
...
\begin{theorem}[异化降低$N$]\label{thm:alienation-lowers-n}
异化态的反馈算子为$U_{22}\circ\pi_!$，其中$\pi_!$是通过!-模型的投影（非满射，丢失线性状态的不可复制部分）。$\rho(U_{22}\circ\pi_!)\leq\rho(U_{22})$，且$\pi_!$非满射时严格不等。
\end{theorem}

\begin{theorem}[革命=消除!-寄生Cut]\label{thm:revolution-cut}
...
\begin{theorem}[$G^\omega$是自函子]\label{thm:gomega-endofunctor}
...
\begin{theorem}[生命论基本定理]\label{thm:fundamental}
以下陈述在操作范畴论中是同一个数学结构的面向：
\begin{enumerate}
\item 生命是自指过程$S=f(S)$（践演元公理）
\item 生命过程是线性的（资源敏感，不可复制）
\item 生命流$\nu F_2$不可被!-资本化（定理\ref{thm:no-bang-stream}）
\item 异化是把!-模型错认为线性过程（定理\ref{thm:alienation-mistake}）
\item 明性是看穿!-模型（定理\ref{thm:mingxing}）
\item 革命是消除!-寄生Cut（定理\ref{thm:revolution-cut}）
\item 量子不可克隆是同一结构在物理领域的显现（定理\ref{thm:no-cloning-corollary}）
\item 慢性死亡是$N<1$时意义有限（定理\ref{thm:slow-death}）
\item 生命过程超越有限证明（定理\ref{thm:life-beyond-proof}）
\end{enumerate}
从"在活着"这个践演事实出发，经线性逻辑和范畴论形式化，必然推出全部九条。
\end{theorem}
...
\begin{theorem}[Composition是定理不是公理]\label{thm:composition-derivable}
