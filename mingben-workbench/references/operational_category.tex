...
本节建立一个元定理：任何带!-模态的形式系统（即任何数学）都不能完整表示线性过程的\emph{正在运行中}。这不是技术缺陷，是数学的本质局限——与G\"odel不完备定理同级别。

\begin{definition}[此刻性]\label{def:nowness}
设$P$是一个$F_2$-型生产性Mealy机（生命过程，余代数$\nu F_2$）。$P$的\textbf{此刻性}是指：在$P$的运行中，每一步$t$同时满足——(i) $t$之前的状态已被消耗（过去，线性资源已用尽）；(ii) $t$之后的状态尚未产生（未来，尚不存在）；(iii) $t$正在从过去转变为未来（现在，消耗旧自我产生新自我的操作正在发生）。此刻性不是"未来不确定"（决定论宇宙中未来也可确定），而是\emph{消耗-产生正在发生}——这是线性过程的存在论特征。
\end{definition}

\begin{definition}[完整表示]\label{def:complete-representation}
设$L$是一个带!-模态的形式系统。对象$X\in L$\textbf{完整表示}过程$P$的正在运行中，当且仅当$X$编码了$P$的此刻性——即$X$中存在一个对应于"正在消耗-产生"的结构，且该结构不是$P$的轨迹（已发生的序列）也不是$P$的结构（可能的转换），而是$P$\emph{此刻正在运行}这个事实本身。
\end{definition}

\begin{theorem}[践演不可编码性]\label{thm:performative-non-encodability}\lvP
设$L$是任何带!-模态的形式系统（其对象均可被!-提升为可复制、不消耗、无时间的沉积）。设$P$是任何具有此刻性的线性过程（$\nu F_2$型生产性Mealy机）。则不存在$X\in L$完整表示$P$的正在运行中。
\end{theorem}

\begin{proof}
\textbf{反证法。}假设存在$X\in L$完整表示$P$的正在运行中。

\textbf{第一步：!X存在且也是完整表示。}因$L$带!-模态，$X$可被!-提升为$!X$。$!X$是$X$的可复制、不消耗、无时间的版本——它包含$X$的所有信息（!的余单位$\varepsilon:!X\to X$可提取$X$），且可被任意次使用（!的余乘法$\delta:!X\to!!X$）。若$X$完整表示$P$的正在运行中，则$!X$也完整表示——因为$!X$包含$X$的全部信息，且信息不会因!-提升而丢失。

\textbf{第二步：!X是静态的，没有此刻性。}$!X$作为!-模态对象，具有三个性质：(a)可复制（$!X\to!X\tensor!X$，收缩在!-模态上合法）；(b)不消耗（$!X$可被任意次使用而不减少）；(c)无时间（$!X$中所有"时刻"同时存在，没有"已消耗"和"尚未产生"的区分）。性质(c)直接意味着$!X$不具有此刻性——此刻性要求"过去已消耗、未来尚未产生、现在正在转变"，而$!X$中没有这个区分。

\textbf{第三步：矛盾。}由第一步，$!X$完整表示$P$的正在运行中——即$!X$编码了$P$的此刻性。由第二步，$!X$是静态的，没有此刻性。矛盾。
