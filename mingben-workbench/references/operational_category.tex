...
\begin{proof}
(1) 每步萃取$\beta(t)$比例后，反馈算子缩放为$U_{22}(t+1)=(1-\beta(t))U_{22}(t)$（加权$\mathbf{Rel}$中萃取=反馈缩放，定理\ref{thm:extraction-alienation}模型条件）。由谱半径齐次性$\rho(cU)=|c|\rho(U)$（$c\geq0$），$N(t+1)=(1-\beta(t))N(t)$，归纳得$N(t)=N(0)\prod_{s=0}^{t-1}(1-\beta(s))$。因$1-\beta(s)\leq1$，$N(t)$单调不增。
