...
\begin{theorem}[G\"odel不完备$=$定理\ref{thm:bang-no-ptc}特例]\label{thm:godel}\lvC
...
$\bang$-余代数构成的死子范畴$\mathcal{C}^!$是笛卡尔的：有终对象$I$，有积$\bang A\otimes\bang B$（Seely同构$\bang(A\times B)\cong\bang A\otimes\bang B$），有自然投影。死结构的世界可以拆分、复制、丢弃——经典逻辑（笛卡尔范畴）描述$\bang$-模态世界是恰当的，但不描述生命。
\end{theorem}
...
在满足以下条件的PTC中$\nu F_2$过程必然涌现：(1)持续线性资源流入（$F\neq\emptyset$）；(2)反应网络超过RAF复杂度阈值（Kauffman 1986, Hordijk--Steel 2004）；(3)$\bang$-模态催化剂存在（定理\ref{thm:catalyst-bang}）。自催化集合中的生产性反馈回路给出$\nu F_2$余代数。生命不是偶然奇迹，是PTC公理在有能量流动处的必然推论。
\end{theorem}
...
$\mathrm{Life}(\mathcal{C})$是对称幺半闭范畴，有GoI迹和守卫递归，但$\bang$是函子$\mathrm{Life}(\mathcal{C})\to\mathcal{C}^!$（从生到死），不是$\mathrm{Life}(\mathcal{C})$上的自函子。在纯粹生命范畴中没有"死亡"的位置——$\bang$-沉积是向死结构范畴的变换，不是生命内部操作。
\end{theorem}
