...
\begin{proposition}[$\nu F=$Fock空间完备化]\label{prop:nu-fock}\lvC
在GoI迹内积$\langle x|y\rangle_{\mathrm{Tr}}=\mathrm{Tr}(x^\dagger y)$下，$\nu F$完备化为全Fock空间$\mathcal{F}(\mathcal{H})=\bigoplus_{n=0}^{\infty}\mathcal{H}^{\otimes n}$（平方可迹条件$\sum_n\langle x_n|x_n\rangle<\infty$）。粒子数算符$a^\dagger a$对应量论的$T$（操作持续步数）。此命题依赖迹内积正定性的验证。
\end{proposition}
