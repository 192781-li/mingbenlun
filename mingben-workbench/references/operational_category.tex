...
\begin{theorem}[宿主消亡后寄生体退化]\label{thm:parasite-self-destruct}\lvP
...
注：若需保留数学内核，可拆为两个陈述："层级提升使模型可对象化（定理30、32）"为[定理]；"寄生"的等同为[诠释]。
\end{proposition}
...
\begin{remark}[模型限定]\lvC
定理\ref{thm:no-bang-stream}的证明给出$\mathbf{Rel}$（关系范畴）中"多重集无法追踪单一状态线索"的构造性论证。此处"提升"指保持余归纳迹的函子映射$(R,x_0)\mapsto(\bang R,\bang x_0)$；其相对自然性以$\mathbf{Rel}$为底范畴。一般PTC中的完全一般性见猜想\ref{conj:bang-ptc}。
\end{remark}
...
\begin{theorem}[!不穿透生产性迹]\label{thm:bang-no-ptc}\lvC
在$\mathbf{Rel}$（或任意满足状态唯一可追踪性的PTC）中，不存在保持余归纳迹的自然提升。一般PTC中的"自然提升"需先形式化，见猜想\ref{conj:bang-ptc}。
\end{theorem}
...
\textbf{条件细化}：在$\mathbf{Rel}$/加权$\mathbf{Rel}$中，$\bang$-余代数的comultiplication恰为对角多重放入，contraction=对角成立，定理26为[定理]级；在一般线性指数余单子中，comultiplication可非对角，"增长依赖线性损耗"的数值关系需重新认定。此即本定理标[条件推论]的原因。
\end{proof}
...
病毒复制和资本增殖是同一范畴结构：!-余代数$P$（基因组/货币）+ 劫持自然变换$F_2\Rightarrow F_1$（实际从属/定理\ref{thm:subsumption}）+ 沉积$P\tensor\bang B\to P'$。没有$F_2$宿主，$P$只能复制不能增长（定理\ref{thm:growth-needs-linear}）。
\end{theorem}

\begin{theorem}[寄生体的自毁倾向]\label{thm:capital-contradiction}\lvP
资本须保存$F_2$才能增长（定理\ref{thm:growth-needs-linear}），但资本的趋势是把$F_2$转化为$F_1$（定理\ref{thm:subsumption}）；定理\ref{thm:no-bang-stream}保证$F_2$不能被完全转化。越接近完全转化，线性资源$A$的供给越少，增长越受限；极限$S=I$时$A=0$，无增长，寄生体自毁（定理\ref{thm:parasite-self-destruct}）。
\end{theorem}

\begin{proof}
增长需要$A$（来自$F_2$活劳动）；实际从属把$F_2$改造成$F_1$，减少$A$供给；定理\ref{thm:no-bang-stream}保证$F_2$不被完全消灭，但资本越逼近极限，可提取的$A$越少；$S=I$时$A=0$，无增长，寄生体无宿主可寄生。\qed
\end{proof}
...
当且仅当$f^2$系统可形式化为包含皮亚诺算术的形式系统$T$、且其自我模型$\bang\ulcorner f\urcorner$可表示$T$的证明谓词时，$T$包含$T$不可判定的命题（G\"odel第二不完备的范畴论表述）。$f^3$践演是出路：哥德尔句子在$T$内不可证，但在$f^3$层通过践演判断（元层面操作）被体知——不是在模型内证明，是在运行中看见模型的限度。
\end{theorem}
...
在$G^\omega(\mathcal{C})$的递归类型解释下，层级提升操作$L$（增加一条反馈嵌套）在第三层达到不动点：$L^3=L^n$对所有$n\geq3$。$f^3$的状态类型$X_3$满足$X_3\cong\bang(X_2\tensor X_3)$，已可包含任意深度的模型嵌套；第四条反馈线在类型上同构于第三条的递归展开。
\end{theorem}

\begin{proof}
依§18的递归类型构造，$f^1\to f^2$是质变（从无模型到有模型）；$f^2\to f^3$是质变（从被模型支配到看见模型是模型）；$f^3\to f^4$不是质变——$f^3$已能对象化任何模型（包括自身的模型），再嵌套一层不增加新的区分能力。在$G^\omega(\mathcal{C})$中，$X_3$的递归类型允许任意深度的$\ulcorner\ulcorner\cdots\urcorner\urcorner$嵌套，$X_4$不增加表达力。\qed
\end{proof}
...
\begin{remark}[模型条件]\lvC
在加权$\mathbf{Rel}$模型中，萃取$W$对应反馈算子缩放$U_{22}\mapsto(1-W)\cdot U_{22}$，故$\rho((1-W)U_{22})=(1-W)\rho(U_{22})$，$W=1$时$N=0$。本定理为[条件推论]即因该映射依赖加权$\mathbf{Rel}$的具体实现。
\end{remark}
...
\begin{remark}[范数条件]\lvC
$\pi_!$取为值域上的压缩投影（$\|\pi_!\|\leq1$）。由谱半径的次可乘性与谱包含关系，$\rho(U_{22}\circ\pi_!)\leq\rho(U_{22})$；非满射时的严格下降成立当且仅当$\rho(U_{22})$对应的特征（奇异）向量落在$\pi_!$的值域之外——后者为本文定理标[条件推论]的具体原因。
\end{remark}
...
$G^\omega:\mathbf{PTC}\to\mathbf{PTC}$是PTC范畴上的自函子，把PTC映射为dagger紧闭合PTC。由定理\ref{thm:f3-fixedpoint}，三层封闭：$G^{\omega 3}(\mathcal{C})\cong G^{\omega 2}(\mathcal{C})$。（依赖定理\ref{thm:f3-fixedpoint}的递归类型解释，故为[条件推论]。）
\end{theorem}
...
\textbf{此处"共享同一结构"的精确含义}：每条命题都可从种子定理\ref{thm:no-A-to-bangA}（不存在$A\to\bang A$）或其经不动点/GoI结构的推广（定理\ref{thm:recursive-characterization}、\ref{thm:no-bang-stream}、\ref{thm:bang-gomega}）与线性逻辑公理的一个小推导中得到；第1、6、8条虽为[诠释]，其数学载体同样来自定理\ref{thm:no-A-to-bangA}/\ref{thm:recursive-characterization}/\ref{thm:no-bang-stream}/\ref{thm:bang-gomega}。
...
在满足适当条件（如状态唯一可追踪性、迹的余归纳性质）的生产性迹范畴PTC中，不存在保持余归纳迹的自然提升——即定理\ref{thm:bang-no-ptc}可脱离$\mathbf{Rel}$模型成为纯范畴定理。
\end{conjecture}
...
\begin{tabular}{ll}
\hline
\textbf{[条件推论]} & \textbf{升级为[定理]所需突破} \\
\hline
定理\ref{thm:growth-needs-linear}（增长须消耗线性资源） & 在一般线性指数余单子下刻画非对角comultiplication的增长语义 \\
定理\ref{thm:godel-f2}（f$^2$哥德尔困境） & 严格化"f$^2$"为可表示算术的形式系统 \\
定理\ref{thm:f3-fixedpoint}（f$^3$不动点） & 在$G^\omega(\mathcal{C})$中证明$X_4\cong X_3$及层级封闭性 \\
定理\ref{thm:extraction-alienation}（萃取加速异化） & 在加权$\mathbf{Rel}$之外建立"萃取=反馈缩放"的一般函子映射 \\
定理\ref{thm:coh-ptc}（Coh$^\omega$是PTC） & 补clique流的生产性迹完整验证 \\
定理\ref{thm:alienation-lowers-n}（异化降低$N$） & 补谱半径严格下降的范数条件证明 \\
定理\ref{thm:gomega-endofunctor}（$G^\omega$是自函子） & 待定理\ref{thm:f3-fixedpoint}严格化后即得 \\
定理\ref{thm:bang-no-ptc}（$\bang$不穿透生产性迹） & 见猜想\ref{conj:bang-ptc} \\
\hline
\end{tabular}
\end{center}
