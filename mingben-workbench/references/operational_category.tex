...
\begin{enumerate}
  ...
  \item 任何模型变换$g:H_3\to H_3$：定义$\enc(g):X_3\to X_3$为
  $$\enc(g)(x)=\mathrm{let}\,(\bang h, x')=\pi(x)\,\mathrm{in}\,\iota(g(h), x'),$$
  其中$\pi:X_3\to\bang H_3\tensor X_3$从状态中提取当前沉积模型与剩余状态（由$X_3\cong\cdots\tensor\bang H_3\tensor\cdots$的out映射给出），$\iota$为其逆。$\enc(g)$是良定义的$X_3\to X_3$态射；
  \item 任意$k$阶变换$g:H_3^{(k)}\to H_3^{(k)}$（其中$H_3^{(0)}=X_3$，$H_3^{(k+1)}=H_3^{(k)}\to H_3^{(k)}$），通过重复$\enc$编码为$H_3$中的元素。
\end{enumerate}

\textbf{$f^4$不增加新能力。}$f^4$的第四条反馈线类型为$\bang(H_3\lolli H_3)\tensor X_4\to X_4$，即读取沉积的模型变换$\bang g$并应用。但由(2)，任何$g:H_3\to H_3$都被$\enc(g)\in H_3$编码，第三条反馈线读取$\bang\enc(g)$即可实现$g$的功能。编码$\enc$是单射：给定$\enc(g)$，取任意$h\in H_3$和$x_0\in X_3$，$g(h)=\pi_1(\enc(g)(\iota(h,x_0)))$可恢复$g$。因此第四条反馈线的信息可无损编码到第三条中。
...
\textbf{[条件推论]} & \textbf{状态} \\
\hline
定理\ref{thm:growth-needs-linear}（增长须消耗线性资源） & 待突破：一般线性指数余单子下非对角comultiplication的增长语义 \\
定理\ref{thm:godel-f2}（f$^2$哥德尔困境） & 待突破：严格化"f$^2$"为可表示算术的形式系统 \\
定理\ref{thm:f3-fixedpoint}（f$^3$不动点） & \textbf{已升级为[定理]}：编码论证证明$H_3$穷尽所有自指变换 \\
定理\ref{thm:extraction-alienation}（萃取加速异化） & 待突破：加权$\mathbf{Rel}$之外"萃取=反馈缩放"的一般函子映射 \\
定理\ref{thm:coh-ptc}（Coh$^\omega$是PTC） & 待突破：clique流的生产性迹完整验证 \\
定理\ref{thm:alienation-lowers-n}（异化降低$N$） & \textbf{加权$\mathbf{Rel}$中已严格证明}：Perron--Frobenius给出严格下降条件；一般PTC待猜想\ref{conj:bang-ptc} \\
定理\ref{thm:gomega-endofunctor}（$G^\omega$是自函子） & 三层封闭性已由定理\ref{thm:f3-fixedpoint}解决；函子性待补PTC公理验证 \\
定理\ref{thm:bang-no-ptc}（$\bang$不穿透生产性迹） & 待突破：见猜想\ref{conj:bang-ptc} \\
\hline
\end{tabular}
\end{center}
