...
\section*{定理级别约定}
\addcontentsline{toc}{section}{定理级别约定}

本文严格区分三个级别：
\begin{itemize}
\item \textbf{[定理]}：在公理1--4（SMCC+!余单子+不动点）下有严格证明的数学命题，不依赖哲学预设。
\item \textbf{[条件推论]}：依赖特定模型条件（如Rel中的对角contraction、加权矩阵的谱半径）或尚未完全严格证明的命题。条件会明确标注。
\item \textbf{[诠释]}：数学结构与哲学/政治经济学概念的对应。这是赋值和解释，不是数学推导。"$X$对应$Y$"不声称"$X$就是$Y$"。
\end{itemize}

公理5（践演元预设）是哲学预设，不是数学公理，不用于证明[定理]级命题。
...
\begin{theorem}[活自指良基]\label{thm:productive-self-ref}\lvI
...
\begin{theorem}[寄生体自毁]\label{thm:parasite-self-destruct}\lvP
...
\begin{theorem}[剩余价值的线性来源]\label{thm:surplus-linear}\lvP
...
\begin{theorem}[生命不可完全资本化]\label{thm:life-not-capital}\lvP
...
\begin{theorem}[商品拜物教]\label{thm:fetishism}\lvI
...
\begin{theorem}[有机构成上升]\label{thm:organic-composition}\lvI
...
\begin{proposition}\label{prop:f3-alienation}\lvI
...
\begin{theorem}[形式从属与实际从属]\label{thm:subsumption}\lvP
...
\begin{theorem}[生命流不可资本化]\label{thm:nu-no-lift-life}\lvP
...
\begin{theorem}[增长必须消耗线性资源——模型相关]\label{thm:growth-needs-linear}\lvC
\textbf{在!-contraction为对角映射的模型中}（$\mathbf{Rel}$、$\mathbf{Coh}$：$\bang G\to\bang G\tensor\bang G$把$g$映射为$(g,g)$，两副本相同），不存在纯从$\bang G$到$\bang G\tensor g$（$g>0$）的态射使价值增长而不消耗线性资源。增长需要$A\tensor\bang G\to\bang G\tensor g$型态射，其中$A$是线性资源。
\end{theorem}

\begin{proof}
在对角contraction模型中，!-态射只能复制（定理\ref{thm:copy-not-growth}），不能产生增量——这是contraction定义的直接推论，不是额外假设。若$G$是线性的，$G\to G\tensor g$消耗$G$后$G$不存在，无法持续。要持续增长必须有外部线性资源$A$被消耗。

\textbf{条件性说明}：本定理依赖"contraction=对角复制"这一模型条件。在一般操作范畴中，!-余代数的counit/comultiplication不一定是对角映射，结论可能不成立。定理\ref{thm:surplus-linear}（剩余价值只能来自线性资源）在相同模型条件下成立。
\end{proof}

\begin{theorem}[$M$-$C$-$M'$是假$F_2$]\label{thm:fake-f2}\lvP
...
\begin{theorem}[病毒与资本同构]\label{thm:virus-capital}\lvP
...
\begin{theorem}[资本的矛盾]\label{thm:capital-contradiction}\lvP
...
\begin{theorem}[异化=错认名字为过程]\label{thm:alienation-mistake}\lvP
...
\begin{theorem}[明性=看见名字是名字]\label{thm:mingxing}\lvP
...
\begin{theorem}[$f^2$的哥德尔困境]\label{thm:godel-f2}\lvC
...
\begin{theorem}[定理21严格化]\label{thm:mu-rigorous}\lvP
...
\begin{theorem}[$f^3$是不动点]\label{thm:f3-fixedpoint}\lvC
...
\begin{theorem}[萃取加速异化]\label{thm:extraction-alienation}\lvC
...
\begin{theorem}[有限片段完全性]\label{thm:completeness-finite}\lvP
在有限非交互片段（$F_1$型，$\mu$不动点）中，GoI解释对MELL片段完全：每个nilpotent连线对应一个证明（Girard 1989，Danos-Regnier 1995）。
\end{theorem}

\begin{theorem}[生命过程超越证明]\label{thm:life-beyond-proof}\lvP
...
\begin{theorem}[Coh$^\omega$是PTC]\label{thm:coh-ptc}\lvC
...
\begin{theorem}[$G^\omega$是自函子]\label{thm:gomega-endofunctor}\lvC
...
\begin{enumerate}
\item 生命是自指过程$S=f(S)$（践演元预设，[诠释]）
\item 生命过程是线性的（资源敏感，不可复制）（[定理]）
\item 生命流$\nu F_2$不可被!-资本化（定理\ref{thm:no-bang-stream}，[定理]）
\item 异化是把!-模型错认为线性过程（范畴论区分是[定理]，与"异化"的等同是[诠释]）
\item 明性是看穿!-模型（同上）
\item 革命是消除!-寄生Cut（GoI的Cut消除是[定理]，与"革命"的等同是[诠释]）
\item 量子不可克隆是同一结构在物理领域的显现（定理\ref{thm:no-cloning-corollary}，[定理]）
\item 慢性死亡是$N<1$时$M$有限（几何级数是[定理]，"$M$=意义"是[诠释]）
\item 生命过程超越有限证明（定理\ref{thm:life-beyond-proof}，[定理]）
\end{enumerate}
\textbf{不声称}"从践演事实必然推出全部九条"。声称的是：九条命题共享同一个数学结构（线性资源不可被!-模态还原），其中第2、3、7、9条有严格证明（[定理]）；第4、5条的范畴论区分是[定理]，与"异化""明性"的等同是[诠释]；第1、6、8条是[诠释]——数学结构与哲学概念的对应不是推导，是赋值。
\end{theorem}
