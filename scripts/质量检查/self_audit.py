...
# 路径锚定
SCRIPT_DIR = Path(__file__).parent  # scripts/质量检查/
SCRIPTS_DIR = SCRIPT_DIR.parent        # scripts/
WORKSPACE = SCRIPTS_DIR.parent         # 仓库根
SKILL_DIR = WORKSPACE                  # 兼容旧变量名，指向仓库根
MODDIR = WORKSPACE / "生命论_模块化"
FULL_MD = WORKSPACE / "build_output" / "生命论_合订本.md"

def is_archived(path):
    """检查路径是否在归档目录中"""
    s = str(path)
    return 'archive/' in s or '/archive/' in s

def safe_rglob(directory, pattern):
    """rglob但真正跳过archive/目录（用os.walk，不遍历归档目录）"""
    import os
    suffix = pattern.replace('*', '')
    for root, dirs, files in os.walk(directory):
        # 跳过archive/和.git/目录
        dirs[:] = [d for d in dirs if d not in ('archive', '.git', '__pycache__', 'build_output')]
        for fname in files:
...
def check_chapter_count_consistency():
    ...
    targets = list(safe_rglob(SKILL_DIR, "*.md")) + list((WORKSPACE / "docs").rglob("*.md"))
...
def check_empty_dirs():
    """[空目录] 技能目录下不应有空目录"""
    bad = []
    for skill_dir in [SKILL_DIR, WORKSPACE / "docs"]:
...
def check_outdated_keywords():
    ...
    bad = []
    for f in list(safe_rglob(SKILL_DIR, "*.md")) + list((WORKSPACE / "docs").rglob("*.md")):
        text = f.read_text(encoding='utf-8', errors='replace')
