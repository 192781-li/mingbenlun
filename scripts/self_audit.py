...
# 路径锚定（支持 MINGBEN_WORKSPACE 环境变量注入项目目录；未注入时回退到与技能相邻的旧假设）
SCRIPT_DIR = Path(__file__).parent
SKILL_DIR = SCRIPT_DIR.parent
PROJECT_ENV = os.environ.get("MINGBEN_WORKSPACE")
if PROJECT_ENV:
    WORKSPACE = Path(PROJECT_ENV)
else:
...
def safe_rel(path):
    """安全相对路径：技能文件可能不在项目内，避免 relative_to 抛异常"""
    try:
        return str(path.relative_to(WORKSPACE))
    except ValueError:
        return str(path)

# 与 build.sh 的 manifest 完整性检查保持一致的排除规则
def is_work_doc(rel):
    """判断是否为不入 manifest 的工作文档（与 build.sh 规则对齐）"""
    if rel.startswith('项目文档/'):
        return True
    if rel.startswith('副卷_语义论/'):
        base = os.path.basename(rel)
        return not (base.startswith('篇') or base.startswith('00_'))
    return False
    ...
    for f in targets:
        ...
        for m in re.finditer(r'(\d{2,3})\s*章', text):
            ...
            if any(kw in ctx for kw in ['全本', '总章', '共', '模块', '当前', '个章']):
                if claimed != actual:
                    fail(f"章数不一致 {safe_rel(f)}: 声称{claimed}章，实际{actual}章")
                    break
        else:
            continue
        break
    ...
    for f in targets:
        ...
        for m in re.finditer(r'附录\s*([一二三四五六七八九十\d]+)\s*种', text):
            ...
            if claimed is not None and claimed != actual:
                fail(f"附录数不一致 {safe_rel(f)}: 声称{claimed}种，实际{actual}种")
                bad = True
    if not bad:
    ...
    for scan_dir in scan_dirs:
        for f in scan_dir.rglob("*.md"):
            ...
            for m in ref_pattern.finditer(text):
                ...
                # 解析路径：先试引用文件自身所在目录，再试workspace根、scripts/、技能目录
                candidates = [f.parent / ref, WORKSPACE / ref,
                              WORKSPACE / "scripts" / ref, scan_dir / ref]
                if not any(c.exists() for c in candidates):
                    bad.append(f"{safe_rel(f)} → {ref}")
    if bad:
        for b in bad:
            fail(f"失效引用: {b}")
    else:
...
def check_cross_references():
    ...
    for m in re.finditer(r'第([零一二三四五六七八九十百千两]+)章', text):
        # 排除外部文献章号：《…》书名内，或"《…》第X卷 第X章"（如《资本论》第一卷第二十四章）
        pre = text[max(0, m.start()-50):m.start()]
        if re.search(r'《[^》]*》\s*第[零一二三四五六七八九十百千两]+卷\s*$', pre):
            continue
        if '《' in pre and '》' not in pre[pre.rfind('《'):]:
            continue
        # 排除"教材/教科书/课本/拟稿/小说 第X章"等外部、草稿或文学指称
        if re.search(r'(教材|教科书|课本|拟稿|小说)[^。；]{0,8}$', pre):
            continue
        n = cn2int(m.group(1))
    ...
    for skill_dir in [SKILL_DIR, SKILL_DIR.parent / "mingben-output"]:
        ...
        for d in skill_dir.rglob("*"):
            if d.is_dir() and not any(d.iterdir()):
                if '.git' in d.parts:
                    continue  # git内部结构目录不是空壳问题
                bad.append(str(safe_rel(d)))
    if bad:
        for b in bad:
            fail(f"空目录: {b}")
    else:
...
def check_outdated_keywords():
    ...
    for f in list(SKILL_DIR.rglob("*.md")) + list((SKILL_DIR.parent / "mingben-output").rglob("*.md")):
        if 'archive' in f.parts:
            continue  # 归档目录是历史存档，不参与关键词扫描
        text = f.read_text(encoding='utf-8', errors='replace')
        for kw, desc in outdated.items():
            if kw in text:
                bad.append(f"{safe_rel(f)} 含 '{kw}'（{desc}）")
    if bad:
        for b in bad:
            fail(f"过时关键词: {b}")
    else:
...
def check_manifest_completeness():
    ...
    for f in MODDIR.rglob("*.md"):
        ...
        if is_work_doc(rel):
            continue  # 项目文档/副卷工作文档不入manifest（与build.sh对齐）
