...
2. 引理来源四分类（大胆用已证、绝不碰幻觉，不重复造轮子）：
   (i) 材料 Layer1/Layer2 已 Qed 的：直接用；
   (ii) Coq 标准库/知名库已机器证明者（已 Require List PeanoNat Lia ClassicalEpsilon，如 app_nil_r、List.length_app/removelast、Nat.lt_*、Nat.eqb_* 等）：【大胆直接用，不要重证】，用前确认其命题与目标精确匹配，首次处打 (* @stdlib 库.模块.名 L1 *)；
   (iii) 教材/论文里有、但本库无机器证明的结论：不能直接 apply（Coq kernel 不认纸设定理），按 (* @cite 来源 L2 *) 标思路并在本库重证到 Qed；
   (iv) 查无实据、你也说不清出处的名字=幻觉，禁止；宁可自己立 (* @prove *) 小引理当轮证掉。
   另：加强归纳/证更强引理（把目标 strengthen 成更强命题以获得够用的归纳假设，或 mutual 同时证正反两方向）是【自由且推荐的证明技术】，不是外部假设、无需上报；cong_sym/cong_trans 这类反转方向就用它。
...
7. 【明性——先练会再动手，交付前自我终裁，不等执行方检查】
   ...
   (e) 拼装优先、戒节外生枝（fcell→pick→choose_none_iff 连环弯路的教训）：主证明里某个局部 tactic 报错时，就地修这一步，优先用已 Qed 零件把“目标命题成立”拼出来；【不要为绕开一个局部错而去新造更大的抽象引理、选择函数或更强等式】。确需新引理时，先在最小格（option 三态、nil/单元素）枚举确认它【为真】再写——iff/双向引理必须正、反两个方向各取一个实例验证，任一方向找到反例就一票否决（choose_none_iff 反向反例：左-左格 choose=g2=None 时 g 仍可在位）。
   (f) 追求命题成立，不追求字面相等：exists/构造类目标只需证目标关系（如 cell_split）成立，不要求逐位字面相等。看清 None（越界）与 Some None（在位空）在空位析取里的异同：需要“越界 None”时，固定长度的在位表在 n<长度 位给不出 None，此时要靠【长度/前缀结构】（让需要 None 的位落入越界后缀）解决，而不是把 None 强换成 Some None——后者在“g 越界、邻位在位空”格会让 cell_split 两支皆破。
